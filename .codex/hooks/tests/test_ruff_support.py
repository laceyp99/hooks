...
def test_repo_uses_ruff_when_config_file_exists(
    load_script_module, tmp_path, name: str
) -> None:
...
def test_repo_uses_ruff_when_metadata_mentions_ruff(
    ...
    ruff_support = load_script_module(
        "scripts/ruff_support.py", "ruff_support_metadata"
    )
...
def test_repo_uses_ruff_returns_false_without_markers(
...
def test_file_mentions_ruff_returns_false_for_missing_file(
    load_script_module, tmp_path
) -> None:
...
def test_file_mentions_ruff_returns_false_on_read_error(
    ...
    ruff_support = load_script_module(
        "scripts/ruff_support.py", "ruff_support_read_error"
    )
